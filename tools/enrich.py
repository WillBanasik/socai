...
PROVIDERS: dict[str, list] = {
    ...
    "domain": [_vt_lookup, _urlscan_lookup, _urlhaus_lookup, _threatfox_lookup, _censys_lookup, _otx_lookup, _whoisxml_lookup, _opencti_lookup],
    "url":    [_vt_lookup, _urlscan_lookup, _urlhaus_lookup, _threatfox_lookup, _otx_lookup, _opencti_lookup],
...
PROVIDERS_DOMAIN_FAST: list = [_urlhaus_lookup, _threatfox_lookup, _opencti_lookup, _whoisxml_lookup]
PROVIDERS_DOMAIN_DEEP: list = [_vt_lookup, _urlscan_lookup, _censys_lookup, _otx_lookup]

# Tiered enrichment for URLs — fast/free first, deep only if signal detected
PROVIDERS_URL_FAST: list = [_urlhaus_lookup, _threatfox_lookup, _opencti_lookup]
...
def enrich(case_id: str, max_per_type: int = 20, skip_iocs: set[str] | None = None) -> dict:
    ...
    # =====================================================================
    ip_list = iocs_data.get("iocs", {}).get("ipv4", [])
    if ip_list:
        # Filter known-clean domains (for IPs this is a no-op, but keep consistent)
        filtered_ips = [i for i in ip_list if not _is_known_clean(i, "ipv4")]
        if skip_iocs:
        ...
        for ip in filtered_ips:
            ...
            if info:
                ...
                if owner:
                    infra_ips.append(ip)
                    # Record as infra_clean result (no API calls spent)
                    all_results.append({
        ...
        # --- Tier 1: Fast providers (AbuseIPDB, URLhaus, ThreatFox, OpenCTI) ---
        if candidate_ips:
            ...
            # Reload cache after tier1 writes
            with _cache_lock:
                cache = _cache_load()
    ...
    if domain_list:
        ...
        if filtered_domains:
            # --- Tier 1: Fast providers (URLhaus, ThreatFox, OpenCTI, WhoisXML) ---
    ...
    if url_list:
        ...
        if filtered_urls:
            # --- Tier 1: Fast providers (URLhaus, ThreatFox, OpenCTI) ---
    ...
    # =====================================================================
    # Hashes (MD5, SHA1, SHA256) — Tiered enrichment
    # =====================================================================
    _HASH_TYPES = ("md5", "sha1", "sha256")
    for hash_type in _HASH_TYPES:
        ...
        # Select the appropriate deep provider list per hash type
        if hash_type == "sha256":
        ...
        # --- Tier 1: Fast malware DBs (MalwareBazaar, ThreatFox, OpenCTI) ---
        # MalwareBazaar only supports MD5/SHA256, but the function handles skipping internally
    ...
    # =====================================================================
    _TIERED_TYPES = {"ipv4", "domain", "url"} | set(_HASH_TYPES)
