...
def _fetch_feeds(days: int = 7, category: str | None = None) -> list[dict]:
    ...
    for src in sources:
        if src.get("type") != "rss":
            continue
        try:
            resp = requests.get(src["url"], headers=headers, timeout=_REQUEST_TIMEOUT)
...
def generate_articles(
    ...
    for group in groups:
        ...
        md_manifest = write_artefact(art_dir / "article.md", md_text)
        ...
        if case_id:
            ...
            write_artefact(case_art_dir / f"{art_id}.md", md_text)
