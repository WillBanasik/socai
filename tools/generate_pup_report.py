...
--------------------------
LLM-assisted PUP/PUA (Potentially Unwanted Program/Application) report
following the same evidence-first analytical standards as MDR reports but
framed for unwanted software rather than active compromise.
...
def generate_pup_report(case_id: str) -> dict:
    """
    Generate a PUP/PUA report for *case_id*.
    ...
    if not ANTHROPIC_KEY:
        return {
            "status":  "skipped",
            "reason":  "ANTHROPIC_API_KEY not set — PUP report generation requires LLM access.",
            "case_id": case_id,
            "ts":      utcnow(),
        }
    ...
    try:
        ...
        log_error(case_id, "generate_pup_report.import_anthropic", str(exc), severity="info")
        return {
            "status":  "error",
    ...
    _model = get_model("mdr_report", _severity)  # same tier as MDR reports
    print(f"[generate_pup_report] Querying {_model} for case {case_id}...")
    ...
    out_path = out_dir / "pup_report.md"

    header = (
        f"# PUP/PUA Report — {case_id}\n\n"
    ...
    save_json(out_dir / "pup_report_manifest.json", manifest)

    print(f"[generate_pup_report] Report written to {out_path}")
    print(f"[generate_pup_report] Tokens: {tokens_in} in / {tokens_out} out")
    return manifest
