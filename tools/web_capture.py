...
def _capture_with_playwright(url: str) -> dict:
    """
    Full headless capture via Playwright (opens and closes its own browser).
...
def web_capture_batch(urls: list[str], case_id: str) -> list[dict]:
    ...
    Launching a browser once and reusing its context for every URL eliminates
    per-URL browser startup overhead.  Falls back to serial web_capture() if
