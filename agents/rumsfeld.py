"""
Rumsfeld Agent (Investigation Reasoning Wrapper)
-------------------------------------------------
Wraps ChiefAgent with an LLM reasoning layer:

  Phase A: Run the full existing pipeline (ChiefAgent.run())
  Phase B: Generate investigation matrix (if not already done)
  Phase C: Run determination engine (shadow mode)
  Phase D: Gap analysis — identify unresolved evidence gaps
  Phase E: Propose follow-up actions (analyst approves before execution)

The analyst controls all follow-up actions via CLI or MCP. Nothing is
auto-executed beyond the standard ChiefAgent pipeline.

Usage (programmatic):
    agent = RumsfeldAgent(case_id="IV_CASE_001")
    result = agent.run(title="...", severity="high", ...)
...
logger = logging.getLogger("socai.rumsfeld")


class RumsfeldAgent(BaseAgent):
    name = "rumsfeld"

    def run(self, **kwargs) -> dict:
        """Run the full Rumsfeld investigation pipeline.

        Phase A: Standard ChiefAgent pipeline
        Phase B: Investigation matrix generation
        Phase C: Determination engine (shadow mode)
        Phase D: Gap analysis
        Phase E: Follow-up proposals
        """
        # ==================================================================
        # Phase A: Run the full existing pipeline
        # ==================================================================
        self._emit("phase_start", {"phase": "A", "name": "chief_pipeline"})

        chief = ChiefAgent(self.case_id)
        pipeline_result = chief.run(**kwargs)

        self._emit("phase_done", {"phase": "A",
                                   "ok": sum(1 for s in pipeline_result.get("steps", [])
                                             if s.get("status") == "ok"),
                                   "errors": len(pipeline_result.get("errors", []))})

        # ==================================================================
        # Phase B: Investigation matrix
        # ==================================================================
        matrix = None
        self._emit("phase_start", {"phase": "B", "name": "investigation_matrix"})
        try:
            from tools.investigation_matrix import generate_matrix, load_matrix

            # Check if matrix already generated by chief pipeline
            matrix = load_matrix(self.case_id)
            if not matrix:
                matrix = generate_matrix(self.case_id)

            if matrix:
                self._emit("phase_done", {
                    "phase": "B",
                    "known_knowns": len(matrix.get("known_knowns", [])),
                    "known_unknowns": len(matrix.get("known_unknowns", [])),
                    "hypotheses": len(matrix.get("hypotheses", [])),
                })
            else:
                self._emit("phase_done", {"phase": "B", "status": "skipped",
                                           "reason": "matrix generation failed"})
        except Exception as exc:
            log_error(self.case_id, "rumsfeld.matrix", str(exc),
                      traceback=traceback.format_exc())
            self._emit("phase_done", {"phase": "B", "status": "error"})

        # ==================================================================
        # Phase C: Determination engine (shadow mode)
        # ==================================================================
        determination = None
        self._emit("phase_start", {"phase": "C", "name": "determination"})
        try:
            from tools.determination import llm_determine, compare_dispositions

            determination = llm_determine(self.case_id)
            if determination:
                # Compare with pipeline disposition
                det_disposition = pipeline_result.get("report", {})
                if isinstance(det_disposition, dict):
                    det_disposition = det_disposition.get("auto_disposition")
                comparison = compare_dispositions(
                    det_disposition, determination,
                )
                determination["comparison"] = comparison

                self._emit("phase_done", {
                    "phase": "C",
                    "disposition": determination.get("disposition"),
                    "confidence": determination.get("confidence"),
                    "agrees": comparison.get("agrees"),
                })
            else:
                self._emit("phase_done", {"phase": "C", "status": "skipped"})
        except Exception as exc:
            log_error(self.case_id, "rumsfeld.determination", str(exc),
                      traceback=traceback.format_exc())
            self._emit("phase_done", {"phase": "C", "status": "error"})

        # ==================================================================
        # Phase D: Gap analysis
        # ==================================================================
        gaps = []
        self._emit("phase_start", {"phase": "D", "name": "gap_analysis"})
        try:
            if matrix:
                gaps = _analyse_gaps(matrix)
                self._emit("phase_done", {"phase": "D", "gaps": len(gaps)})
            else:
                self._emit("phase_done", {"phase": "D", "status": "skipped",
                                           "reason": "no matrix available"})
        except Exception as exc:
            log_error(self.case_id, "rumsfeld.gap_analysis", str(exc),
                      traceback=traceback.format_exc())
            self._emit("phase_done", {"phase": "D", "status": "error"})

        # ==================================================================
        # Phase E: Follow-up proposals
        # ==================================================================
        proposals = []
        self._emit("phase_start", {"phase": "E", "name": "followup_proposals"})
        try:
            if gaps:
                proposals = _propose_followups(
                    self.case_id, matrix, gaps, determination,
                )
                if proposals:
                    # Persist proposals for analyst review
                    analysis_dir = CASES_DIR / self.case_id / "artefacts" / "analysis"
                    analysis_dir.mkdir(parents=True, exist_ok=True)
                    save_json(analysis_dir / "followup_proposals.json", {
                ...
                self._emit("phase_done", {"phase": "E", "proposals": len(proposals)})
            else:
                self._emit("phase_done", {"phase": "E", "status": "skipped",
                                           "reason": "no gaps to address"})
        except Exception as exc:
            log_error(self.case_id, "rumsfeld.proposals", str(exc),
                      traceback=traceback.format_exc())
            self._emit("phase_done", {"phase": "E", "status": "error"})
        ...
        self._emit("pipeline_complete", {
            ...
            "determination_generated": determination is not None,
            "gaps_found": len(gaps),
            "proposals_generated": len(proposals),
        })

        # Print summary
        print(f"\n[rumsfeld] Investigation complete for {self.case_id}")
        if matrix:
            kk = len(matrix.get("known_knowns", []))
            ku = len(matrix.get("known_unknowns", []))
            hyp = len(matrix.get("hypotheses", []))
            print(f"[rumsfeld] Matrix: {kk} known, {ku} unknown, {hyp} hypotheses")
        if determination:
            print(f"[rumsfeld] Determination: {determination.get('disposition')} "
                  f"({determination.get('confidence')})")
            comp = determination.get("comparison", {})
            if comp and not comp.get("agrees"):
                print(f"[rumsfeld] ⚠ Determination DISAGREES with pipeline: "
                      f"{comp.get('recommendation')}")
        if proposals:
            print(f"[rumsfeld] {len(proposals)} follow-up proposal(s) pending review")
            print(f"[rumsfeld] Review: socai.py followup --case {self.case_id}")
...
def _llm_propose_followups(
    ...
    try:
        ...
        print(f"[rumsfeld] Follow-up proposals generated "
        ...
        log_error(case_id, "rumsfeld._llm_propose_followups", str(exc),
...
def execute_followup(case_id: str, proposal_id: str) -> dict:
    ...
    try:
        ...
        log_error(case_id, f"rumsfeld.execute_followup.{proposal_id}",
