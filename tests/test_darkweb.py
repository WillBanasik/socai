...
# ---------------------------------------------------------------------------
# Hudson Rock (mocked HTTP)
# ---------------------------------------------------------------------------

class TestHudsonRock:

    @patch("tools.darkweb.HUDSONROCK_KEY", "")
    def test_no_api_key(self):
        from tools.darkweb import hudsonrock_email_search
        result = hudsonrock_email_search(["test@example.com"])
        assert result["status"] == "no_api_key"

    @patch("tools.darkweb.HUDSONROCK_KEY", "test-key")
    @patch("tools.darkweb._hr_is_configured", return_value=True)
    def test_email_search_ok(self, _mock_cfg):
        from tools.darkweb import hudsonrock_email_search
        ...
        mock_resp.json.return_value = {
            "data": [
                {
                    "stealer": "Lumma",
                    "date_compromised": "2024-02-27T10:53:48Z",
                    "ip": "186.18.216.186",
                    "computer_name": "ADMIN",
                    "credentials": [
                        {"url": "https://mail.com", "username": "victim@corp.com",
                         "password": "RealPassword!", "domain": "mail.com"}
                    ],
                }
            ],
        }
        ...
        with patch("tools.darkweb.get_session", return_value=mock_session):
            result = hudsonrock_email_search(["victim@corp.com"])

        assert result["status"] == "ok"
        assert result["compromised_count"] == 1
        # Verify password redacted
        cred = result["results"][0]["credentials"][0]
        assert "REDACTED" in cred["password"]
        assert "RealPassword!" not in json.dumps(result)

    @patch("tools.darkweb.HUDSONROCK_KEY", "test-key")
    @patch("tools.darkweb._hr_is_configured", return_value=True)
    def test_email_search_no_results(self, _mock_cfg):
        from tools.darkweb import hudsonrock_email_search
        ...
        with patch("tools.darkweb.get_session", return_value=mock_session):
            result = hudsonrock_email_search(["clean@safe.com"])
