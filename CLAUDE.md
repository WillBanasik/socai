...
The socai MCP server is wired into this repo's `.mcp.json` (`socai`, SSE on `127.0.0.1:8001`) and runs as an enabled systemd user service (`socai-mcp`, `deploy/socai-mcp.service`) that auto-starts at WSL boot — sessions normally just connect. If `mcp__socai__*` calls fail: probe `curl -s http://127.0.0.1:8001/healthz`, then `systemctl --user restart socai-mcp`, falling back to `python3 -m mcp_server` if the user bus is unreachable (WSL boot race — see "Startup troubleshooting" in `docs/mcp-server.md`). If the server was down when the session started, restoring it is not enough: the client resumes the SSE stream without re-initialising and every call returns `-32602` — only a user-side `/mcp` → reconnect clears it; use the socai CLI / underlying tool functions as the stopgap and say so explicitly. When unsure which mode applies, ask.
...
- **MCP Server** (`mcp_server/`) — HTTP SSE on port 8001 (TLS terminated by an upstream reverse proxy), JWT RBAC; see `docs/mcp-server.md`
