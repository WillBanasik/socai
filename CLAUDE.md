...
| `docs/encore-eql.md` | Encore Gateway EQL: refresh-token auth, Cloudflare UA quirk, scripts/eql_direct.py, `eql-hosted` MCP server (Claude Code `.mcp.json` + Desktop `mcp-remote` wrapper), EQL syntax, case-scoped tools (`eql_entity_context`, `eql_posture_context`, `eql_query`), and **vulnerability hunting** (caseless `eql_vuln_hunt` + `vulnerability-hunting` playbook + `vuln_hunt_report` deliverable; EQL boolean-WHERE quirk) |
