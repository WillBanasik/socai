...
For inline HITL use, these socai MCP tools wrap EQL so the analyst never hand-resolves clients, hand-writes EQL, or hand-logs evidence (four case-scoped tools plus the caseless `eql_vuln_hunt`):
