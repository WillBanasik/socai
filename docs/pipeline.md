...
**Rule-9 early warning (at prepare time):** the `prepare_*` tools for gated types (`prepare_mdr_report`, `prepare_pup_report`, `prepare_executive_summary`, `security_arch_review`) return `evidence_entries` / `finding_entries` counts in their response, plus a `rule9_warning` when either is zero — so a missing chain surfaces *before* the report is written, not as a save-time refusal after the prose already exists. Backfill on the warning, then proceed to the write prompt.

**Quality warnings (at save time, non-blocking):** every successful `save_report` runs the deterministic prose checks from the quality gate (causal-language and speculative-language detection) on the saved text and returns the flags as `quality_warnings` (with `quality_warning_count`) in the manifest. These never block the save — review the flags and revise the deliverable if a causal claim or speculation slipped through. The matrix-backed checks (confirmed-claims, coverage) remain in `review_report_quality`.
