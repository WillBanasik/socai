...
All LLM reasoning is handled by the analyst's local Claude client (Claude Desktop or Claude Code in the terminal). The MCP server has no Anthropic API key and makes no LLM calls.
...
- **Prompts** — load instructions + case data into the analyst's local Claude session for report generation, analytical reasoning, and quality review.
...
Use `127.0.0.1`, not `localhost`, under WSL2.

3. **Mint a token and inject it.** Claude Code does **not** read the project `.env`, so a bare `${SOCAI_MCP_TOKEN}` in `.mcp.json` expands to empty unless the variable is present in Claude Code's environment. The reliable channel is the gitignored `.claude/settings.local.json` `env` block (Claude Code uses it for `${VAR}` expansion):

```bash
# Long-lived operator token; admin bypasses per-tool RBAC. Do not print it into a shared transcript.
SOCAI_JWT_TTL_HOURS=720 python3 -c "from api.auth import create_access_token; print(create_access_token('you@yourco.com','admin',['admin']))"
```
