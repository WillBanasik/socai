...
- **Save tools** — persist output generated by the local agent: `save_report`, `save_threat_article`, `save_analysis`. Handle defanging, HTML conversion, auto-close, and audit logging.
...
| `save_analysis` | `investigations:submit` | Persist structured analysis (determination, matrix, quality gate) to case artefacts. No LLM call. |
...
|---|---|---|
| `run_determination` | Evidence-chain disposition analysis (TP/BP/FP) | `save_analysis` |
...
| `write_timeline` | Forensic timeline reconstruction with MITRE mapping | `save_analysis` |
| `write_evtx_analysis` | EVTX attack chain analysis and narrative | `save_analysis` |
| `write_phishing_verdict` | Phishing page assessment and brand impersonation | `save_analysis` |
| `write_pe_verdict` | PE binary malware assessment | `save_analysis` |
| `write_cve_context` | CVE contextualisation and patching priorities | `save_analysis` |
