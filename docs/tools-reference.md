...
### Tier 3 — Purpose Analysis (via `write_phishing_verdict` prompt)
...
- Auto-close happens on `save_report(report_type="pup_report")` (resolves disposition `pup_pua`) — **not** in `generate_pup_report` itself
- CLI: `python3 socai.py pup-report --case IV_CASE_001` now redirects to the MCP prompt workflow (`write_pup_report` → `save_report`); it does not generate a report directly
...
`config/analytical_guidelines.md` governs analytical reasoning about detections. Embedded verbatim into the MDR, PUP, and sec-arch system prompts (`write_mdr_report`, `write_pup_report`, `write_security_arch_review`) and served live as the `socai://analytical-standards` resource. Key principles: evidence-first analysis, mandatory alternative-explanation evaluation, co-occurrence != causation, precise determination language (Confirmed / Assessed / Unknown), verify-before-asserting (rule 8), and the evidence/finding logging requirement (rule 9). The compact `_ANALYTICAL_STANDARDS` block in `mcp_server/prompts.py` is the short form of the same rules, injected into the investigation and verdict prompts (`hitl_investigation`, `kql_investigation`, `cql_investigation`, `write_phishing_verdict`, `write_pe_verdict`, `write_cve_context`, `user_security_check`).

## Save Tools (Client-Side Persistence)

Three MCP tools persist output generated by the local Claude agent:

- **`save_report`** — persists a report generated via any `write_*` prompt. Accepts markdown (`.md` is the canonical on-disk format). Handles IOC defanging, auto-close (for MDR/PUP/FP deliverables), and audit logging. Returns the persisted markdown as `report_md` so the analyst can render it as a markdown artifact in the Claude Desktop visualiser. Every save also runs the deterministic prose checks from the quality gate (causal-language, speculative-language) and returns the flags as non-blocking `quality_warnings` in the manifest. No LLM call. Reports are immediately available via `socai://cases/{case_id}/report` after saving.
...
## Additional MCP Tools (index)

These registered tools are not detailed above; one-line summaries (toolset group in brackets). See `socai://capabilities` for the live, authoritative list.

- **`query_cyberint_alerts`**, **`cyberint_alert_artefact`**, **`cyberint_metadata`** *(intel)* — Cyberint (Argos) managed-threat alert query, per-alert artefact pull, and account metadata.
- **`query_opencti`**, **`post_opencti_report`**, **`generate_opencti_package`** *(intel)* — OpenCTI STIX query, report publishing (gated by `SOCAI_OPENCTI_PUBLISH`), and STIX 2.1 bundle/package generation. See `docs/opencti-enhancements.md`.
- **`search_confluence`** *(intel)* — search the Performanta Confluence knowledge base.
- **`run_client_exposure_test`**, **`get_client_exposure_report`** *(exposure)* — proactive external-exposure assessment for a client and its cached report.
- **`generate_weekly`** *(admin/intel)* — weekly threat-landscape / activity roll-up.
- **`check_log_coverage`**, **`can_investigate_attack`**, **`refresh_log_coverage`** *(coverage)* — per-client log-source coverage map, attack-type investigability check against that coverage, and coverage-cache refresh.
- **`link_cases`**, **`merge_cases`**, **`discard_case`** *(core)* — case-graph linking, merge of duplicate cases, and discard of a case opened in error.
- **`audit_user_activity`** *(audit)* — structured audit pull of a user's recent activity across available platforms.
